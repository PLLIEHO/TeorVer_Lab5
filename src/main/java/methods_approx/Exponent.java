...
public class Exponent implements Countable{
    public MethodAnswer prepare(double[] arr_x0, double[] arr_y0) {
        ...
        double a = (sxy-(sxx*(sy/sx)))/(sx-((n/sx)*sxx));
        double b = (sy/sx) - (n/sx)*a;
        a = Math.exp(a);
        MethodAnswer answer =  count(arr_x0, arr_y0, a, b);
        answer.setPirson(0);
        answer.setDescriptor(3);
        answer.setA(a);
        answer.setB(b);
        return answer;
    }
